/**
 * 🚀 FINAL GEMINI VISION - Using built-in Node.js modules only
...
export default async function handler(req, res) {
    ...
    try {
        console.log('🚀 Starting Final Gemini Vision Analysis...');
...
Remember: NO HOLD signals allowed - always choose BUY or SELL based on the strongest indicators.`;
        ...
        // Prepare request body for direct HTTP call
        ...
        console.log('🤖 Sending HTTPS request to Gemini API...');

        // Make HTTPS request using built-in Node.js modules
        const geminiResponse = await makeHttpsRequest(apiKey, requestBody);
        ...
        console.log('✅ Gemini analysis received via HTTPS');
        ...
        console.error('❌ Final Gemini Vision analysis failed:', error);

        const processingTime = Date.now() - startTime;

        return res.status(500).json({
            success: false,
            error: 'Final Gemini Vision trading chart analysis failed. Please try again.',
            details: error.message,
            code: 'INTERNAL_ERROR',
            processingTime: processingTime,
            timestamp: new Date().toISOString(),
            service: 'Final Gemini Vision',
            analysisMethod: 'Final Gemini Vision with NO HOLD Guarantee'
        });
    }
}

function makeHttpsRequest(apiKey, requestBody) {
